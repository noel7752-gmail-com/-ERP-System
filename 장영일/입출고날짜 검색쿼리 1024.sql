...
where 1=1
  and INPUT_DATE > to_date('%${input_date_year_first}%-%${input_date_month_first}%','YYYY-MM')
)
</if>
...
from ID_M_INPUT_DATE) entire
where 1=1
  and INPUT_DATE < to_date('%${input_date_year_last}%'-'%${input_date_month_last}%','YYYY-MM')
)
</if>
...
where 1=1
  and OUTPUT_DATE > to_date('%${output_date_year_first}%'-'%${output_date_month_first}%','YYYY-MM')
)
</if>
...
from ID_M_OUTPUT_DATE) entire
where 1=1
  and OUTPUT_DATE < to_date('%${output_date_year_last}%'-'%${output_date_month_last}%','YYYY-MM')
)
</if>
